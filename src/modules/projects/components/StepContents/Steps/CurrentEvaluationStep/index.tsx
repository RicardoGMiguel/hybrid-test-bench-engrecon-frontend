...
interface CurrentEvaluationStepProps {
  projectId: string;
  readOnly: boolean;
}

const CurrentEvaluationStep: React.FC<CurrentEvaluationStepProps> = ({
  projectId,
  readOnly,
}) => {
  ...
  const ControlEmployees = useMemo(() => {
    if (evaluations) {
      const employeesItems: IGroupEmployeeItem[] =
        evaluations.control_employees?.map((item) => ({
          id: item.id,
          name: item.employee.name,
          employeeEvaluation: item || undefined,
        }));
  ...
  const contentToRender = useMemo(() => {
    if (selectedEvaluationType === EvaluationTypeEnum.AUTO_EVALUATION) {
      return (
        <AutoEvaluationContent
          ...
          controlEmployees={ControlEmployees}
    ...
    if (selectedEvaluationType === EvaluationTypeEnum.SENSORS) {
      return <SensorsContent projectId={projectId} readOnly={readOnly} />;
    }
    return undefined;
  }, [
    ControlEmployees,
...
export default CurrentEvaluationStep;
