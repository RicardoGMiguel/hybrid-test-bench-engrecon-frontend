...
import {
  CreateProjectEmployeeFormData,
  createProjectEmployeeFormResolver,
} from './createProjectEmployeeForm.zod';
import {
  ActionsContainer,
  Container,
  Content,
...
const CreateProjectEmployees: React.FC = () => {
  ...
  const { setTempCreateProjectEmployees, tempCreateProjectEmployees } =
    useProjectEmployee();
  ...
  const {
    ...
    watch,
    formState: { isValid },
  } = useForm<CreateProjectEmployeeFormData>({
    resolver: createProjectEmployeeFormResolver,
    defaultValues: tempCreateProjectEmployees,
  });
  ...
  const onSubmit = useCallback(
    async (data: CreateProjectEmployeeFormData) => {
      try {
        setIsLoading(true);

        setTempCreateProjectEmployees(data);
        navigate(PrivatePathsEnum.CREATE_PROJECTS);
        setIsLoading(false);
      } catch (error) {
        setIsLoading(false);
      }
    },
    [navigate, setTempCreateProjectEmployees]
  );
...
export default CreateProjectEmployees;
